...
Each partition has its own physical address space that's allocated to it to ensure safe isolation between each partition. To appropriately manage that, the idea of `SLAT` (**Second Layer Address Translation**) came into play. For clarification, the term `SLAT` is a general term for second layer translation and `EPT` is intel's VT-x implementation of `SLAT`. Each address that's being accessed by a Virtual Processor, is going through 2 address translation processes:
...
The fields that reside in VMCS according to intel:
...
- `VMENTRY control field`s - These fields control `VMENTRY`.
...
We can see that the first argument in **rcx** is [rsp+0A8h+arg_18], which is being built during the function implementation, and the second and third arguments being filled in **rdx** and **r8** registers. The **Call Code** is set relatively at the start and being set to **0x48**:
...
In a case where code is executing in `VTL 1` and the **IRQL** of the interrupt that's being executed is higher than 4 (VINA), no interrupt coming to `VTL 0` will be able to receive priority over the currently executing interrupt. This means that even if an important interrupt would be invoked to `VTL 0`, a VINA interrupt wouldn’t be able to be injected, since the **IRQL** of `VTL 1` is higher in its priority over the **IRQL** of a VINA interrupt that will be injected.

The next thing I'd like to do is to demonstrate the **VINA handler** routine. But before that, it’s important to know that most of the registers are shared between when transitioning from `VTLs` (besides **RIP**, **RSP**, sensitive **CR** registers and sensitive **MSRs**), especially when transitioning from `VTL 0` to `VTL 1` using a “Secure Call”. Whenever transitioning from `VTL 1` to `VTL 0` in **“Normal Calls”**, the **“Secure Kernel”** makes sure to perform the appropriate cleaning of registers before transitioning to prevent potential vulnerabilities or data disclosure that can potentially arise from sharing some registers in `VTL 0`.
...
The `ShvlVinaHandler()` allocates an internal data structure that’s used in both **“Normal Calls”** and **“Secure Calls”** and according to “Windows Internals 7th edition Pt.2” book: **“SKCALL** is 104-byte (**0x68**) data structure used to describe the kind of operation (invoke service, flush TB, resume thread, or call enclave), the secure call number, and a maximum of twelve 8-byte parameters.”
...
The `ShvlpVtlReturn()` is responsible to invokes a **“VTL Return”**, which is responsible through the hypervisor to transition the execution to `VTL 0` and land in the **“Interrupt Service Routine”** in `VTL 0` that caused the hypervisor to inject the **VINA interrupt**.
...
The `NtQueryVirtualMemory()` holds 6 arguments and prepares the argument for the `SkMmQueryVirtualMemory()`, which also has 6 arguments:
...
After that, the syscall index in **“rax”** is passed into the **“rcx”** register as the first argument for the `SkSyscall` and the base address of the expanded stack is passed into the **“rdx”** register as the second argument.
...
The function starts with the allocation of the already-familiar `SKCALL` data structure. and the 12 `QWORD` arguments that will be copied into the `SKCALL` data structure:
...
After that, an allocation to `SkAllocateSyscallBuffer()` is performed, which is used to allocate the mutual buffer to transfer additional data:
...
Most of the functions that invoke a **“Secure Call”** will start with the `Vsl` prefix in their name.
...
In this case, my assumption is that the value **“2”** is to **“Invoke Service”**, which is the secure thread creation s**ecure service**.
...
The function starts by checking if the QWORD global variable pointer called `HvlpVsmVtlCallVa` is initialized. `HvlpVsmVtlCallVa` is a pointer to an assembly block that’s initialized at runtime when VSM is enabled that’s responsible for performing a `VTL Call` call, which will eventually cause execution to transition to `VTL 1`.
